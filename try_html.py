...
def search():
    try:
        choice = request.args['graph']
        if choice == 'person':
            info = request.args['info']
##            try:
            info = info.decode('utf-8')
            analyze = build_graph_for_person('metadata (2).csv', info)
##            except:
##                info = 'uyy'
            return render_template('result_person.html', info = info, imgname = 'static/graph_person.png', analyze = analyze)
        elif choice == 'socium':
            table_html, analyze = build_graph_for_socium(u'metadata (2).csv')
            return render_template('result_socium.html', imgname = 'static/graph_socium.png', analyze = analyze, table_html = table_html)
    except:
